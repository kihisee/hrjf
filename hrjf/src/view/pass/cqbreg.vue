...
<div class="container">
      ...
      <div class="regbox" v-if="isuser==0">
          <ul>
            ...
            <li class="inputtext" style="margin-bottom:0.25rem" v-if="showcode">
                ...
                <span class="vcodeimg" :style="base64" @click="getvcodeimg"></span>
            </li>
  ...
  <div class="subcont_box">
    <div class="row">
      ...
      <div class="text_box">
        <div class="borderline">
          <p>1、活动期间注册用户首次投资活期宝1000元以上，并锁定30天内不提现即免费送100元购物卡（步步高超市购物卡、中邦卜蜂莲花购物卡二选一）。（例如：投资3000，锁定1000元30天不提现；投资5000元，锁定1000元30天不提现）；</p>
          <p>2、领取方式：投资完成后即可凭在平台预留号码前往永州市冷水滩区双洲路626-628领取；</p>
          <p>3、奖品领取时间:工作日时间9:00—18:00；</p>
          <p>4、本次活动最终解释权归恒瑞金服所有。<span v-if="isios">与苹果无关</span></p>
        </div>
      </div>
    </div>
    <div class="row">
    ...
    <p class="textbig">完成转入投资即可获得100元购物卡</p>
    <p><img src="/static/img/activity/cqb/coupon.jpg"></p>
  </div>
</div>
</template>
...
export default {
  ...
  methods:{
    ...
    getvcodeimg(){
      ...
      .then((data) => {
        _this.base64 = 'background:url(data:image/png;base64,'+data.data.ResultData+') no-repeat center top / 100% 100%;';
      })
    },
    activityuserinfo(){
      let _this = this;
       ...
       .then((data) => {
         if(data.data.ResultCode == 100000){
            ...
            if(data.data.ResultData.IsEnd){
                _this.activetime = false;
                _this.$msg.alert("活动时间已结束！",function(){
                _this.gopage();
              },"下载恒瑞金服APP")
              return
            }else{
              if(typeof _this.$store.state.user === 'undefined'){
                return
              }
              _this.$msg.alert(data.data.ResultMessage)
            }
    ...
    login(){
      let _this = this
        ...
        if(!this.activetime){
          this.$msg.alert("活动时间已结束！",function(){
            this.gopage();
          })
          return
        }
