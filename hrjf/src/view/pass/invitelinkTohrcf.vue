...
<div class="invitebody">
    <div class="bannerimg">
        <img src="../../../static/img/invitelink/regbanner2.jpg">
    </div>
    <div class="shurphone"  v-if="step1">
            <p class="step1p" v-if="isvcode">您的好友{{ usermobile }}向您推荐恒瑞金服</p>
            <p class="step1p" v-else>恒瑞金服送您8888元投资券+2%加息券</p>
  ...
  <div class="invitebottom"   v-if="step1">
      <p class="invitebred">2017© 恒瑞集团旗下理财产品</p>
   ...
   methods:{
    ...
    step2click(){
       ...
       let me = this
        ...
        this.$http(this.$api.reg({"LoginPassword":me.password,"EnterChannel":me.EnterChannel,"TerminalType":2,"MobilePhone":me.MobilePhone,"UserName":"hahah","MobileVCode":me.code,"InviteCode":me.invitecode,"ChannelType":1}))
            .then(function(data){
                    if(data.data.ResultCode==100000){
      ...
      step3click(){
          ...
          console.log(_this.checkapp());
                if(_this.checkapp()=="ios"){
                  //window.location.href="itms-apps://itunes.apple.com/cn/app/id1178579593?mt=8"
                  window.webkit.messageHandlers.Method.postMessage({ "type": "GoToAppStore" });
              }
              if(_this.checkapp()=="android"){
                //window.location.href="http://a.app.qq.com/o/simple.jsp?pkgname=com.jinyang.wealth"
                //window.location.href="market://details?id=com.jinyang.wealth"
                android.downloadHrjfAPK();
              }
