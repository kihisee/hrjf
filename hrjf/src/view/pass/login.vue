...
<div id="loginForm">
  ...
  <div class="fin_wen">
      ...
      <span><router-link to="/findpassword" class="blue_a">忘记密码</router-link> </span>｜ <span><a class="blue_a" @click="gotoreg()">免费注册</a></span>
      </p>
  </div>
</div>
  ...
  data () {
   ...
   methods:{
    changeeye(){
      ...
      if(this.isActive){
       ...
       gotoreg(){
         ...
         console.log(encodeURIComponent(query.back))
        //  let allurl=encodeURIComponent("creditdetail?Id="+query.Id+"&i="+query.i+"&isapp="+_this.isapp)
        //  console.log(allurl)
        //let allurl=query
       _this.$router.push("reg?back="+encodeURIComponent(query.back))
    ...
    created:function(){
      let query = this.$route.query
      let _this=this
      // _this.querygo=encodeURIComponent(query)
      _this.querygo=query
      console.log(_this.querygo)
...
.div_zh .lijitou{background:#de3132;color:#fff;display: inline-block;width: 100%;}
