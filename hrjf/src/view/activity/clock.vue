    ...
    <heads :title="title" bgcolor="#7d00e7"></heads>
      <div class="containerChildren pageonebg" v-show="pageshow==1" :style="{top:haedtop}">
          <div class="checkin_y">
              ...
              <div class="checkin_box">
                  ...
                  <div class="daytext" v-if="clockinfo.ShowType!=2">{{btncont2}}</div>
                  <div class="daytext">每日{{clockinfo.PublishTime.substring(11,16)}}公布打卡情况 </div>
                  <div class="daytext"><a href="javascript:" @click="page3show()">查看我的成绩</a></div>
                  <div class="main_text">
                    <div class="top_bg">
      ...
      <div class="containerChildren pagetwobg" v-show="pageshow==2" :style="{top:haedtop}">
        <div class="ponitokbg_box">
          <div class="ponitok_box">
              <div class="oktext"><p><span>已支付{{clockinfo.UserPayPoints}}积分</span></p><p><span>记得</span>明早打卡才能瓜分积分<span>哦</span></p></div>
      ...
      <div class="confirmbox" v-if="confirm">
          <div class="confirm">
            ...
            <p>确定支付{{clockinfo.UserPayPoints}}积分<br>预约明早打卡瓜分积分？</p>
            <a href="javascript:" class="btn" @click="submitpoint()">确定</a>
            <a href="javascript:" class="btn2" @click="cancel()">取消预约</a>
            <div style="text-align:center;margin-top:0.25rem;color:#fff">取消预约则明早不能参与打卡瓜分积分</div>
          </div>
      </div>
      <div class="confirmbox" v-if="confirm2">
          <div class="confirm2">
            <a href="javascript:" class="cancel" @click="cancel()"></a>
            <p>时间未到<br>请在{{IsToday}}<span>{{clockinfo.BookStartTime.substring(11,16)}}-{{clockinfo.BookEndTime.substring(11,16)}}</span>进行打卡</p>
          </div>
      </div>
      <div class="successbox" v-if="successpage">
          <div class="success">
            ...
            <div class="text">今天可随机瓜分{{clockinfo.CanGainPoints}}积分，{{clockinfo.PublishTime.substring(11,16)}}打卡结束后，可在我的成绩页查看瓜分的积分。</div>
            <!-- <a href="javascript:" class="btn" @click="submitpoint()">明天继续打卡</a> -->
            <a href="javascript:" class="btn" @click="daka()">明天继续打卡</a>
          </div>
      </div>
    </div>
</template>
  ...
  methods:{
    ...
    daka(){
      if(this.clockinfo.ShowType==3){
        //this.$msg.alert("时间未到，请在明天"+this.clockinfo.BookStartTime.substring(11,16)+"-"+this.clockinfo.BookEndTime.substring(11,16)+"进行打卡")
        this.openbg = true;
        this.confirm2 = true;
        return
      }
    ...
    finish(){
      ...
      this.intervalid = setInterval(function () {
          ...
          _this.clocktime=hour+"小时"+minute+"分钟"+second+"秒"
          _this.btncont2 = _this.clocktime+"后才能打卡";
          intDiff--;
          if (intDiff <= 0) {
              if (show) {
                  _this.clockinfo.ShowType=1;
     ...
     getclockdata(){
      ...
      .then((data) => {
        ...
        if(data.data.ResultCode==100000){
          ...
          if(_this.clockinfo.IsToday){
            _this.IsToday = "早晨"
          }else{
            _this.IsToday = "明天"
          }
          if(_this.clockinfo.ShowType==2){
            _this.btncont = "预约明早签到打卡";
            _this.iconclass = "ka"
          }
...
.containerChildren{
  ...
  .confirmbox{
      ...
      .confirm2{
        ...
        height:rem(320);
        margin-top:rem(-100);
        @include bg("/static/img/activity/checkin/alert_ok2.png");
        position:relative;
        a.cancel{
          height:rem(45);
          width:rem(45);
