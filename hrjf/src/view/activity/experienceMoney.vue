...
export default {
          ...
          if(data.data.ResultCode==100000){
              arr.forEach(function(val,index){
                if(val==data.data.ResultData[0].PrizeMoney){
                  ...
                  _this.PrizeId = data.data.ResultData[0].PrizeId
                  _this.PrizeMoney = data.data.ResultData[0].PrizeMoney
                }
              })
          }else if(data.data.ResultCode==80001){

                  _this.PrizeId = data.data.ResultData.prizeId
                  _this.PrizeMoney = data.data.ResultData.prizeMoney

                  console.log(_this.PrizeId ,_this.PrizeMoney )
                 if (query.ver == 'android') {
                      _this.$msg.alert(data.data.ResultMessage,function(){
                       var resutl = android.experiencetender(_this.PrizeId,_this.PrizeMoney);
                       var resutlToken = resutl.toString();
                      },"立即使用")
                      console.log("android")
                      ...
                      _this.$msg.alert(data.data.ResultMessage,function(){
                          ...
                          var resutl = window.webkit.messageHandlers.Method.postMessage({ "type": "experiencetender","PrizeId":_this.PrizeId,"PrizeMoney":_this.PrizeMoney });
                      },"立即使用")
                  }else{
                    console.log("m")
                      _this.$msg.alert(data.data.ResultMessage,function(){
                        _this.$router.push("/tenderdetailnew?id="+_this.PrizeId+"&PrizeMoney="+_this.PrizeMoney )
                      },"立即使用")
  ...
  created:function(){
    let _this = this
    let query = this.$route.query
    if(query.token||typeof this.$store.state.user!="undefined"){
      this.token = true
    }
    this.getInviteCode()
