...
<div class="nav">
  <div class="box"><a href="javascript:" :class="{active:index==0}" @click="tab(0)">新楼盘</a></div>
  <div class="box"><a href="javascript:" :class="{active:index==1}" @click="tab(1)">二手房</a></div>
</div>

<div class="hothouse_box"  id="iscroll" :style="styletop">
  <div id="content">
      <ul v-if="index==1&&houselist.length>0">
    <li v-for="(item,index) in houselist" @click="detail(1,item.HouseId,index)" :class="{out:item.HouseSaleStatus=='已售'||item.HouseSaleStatus=='停售'}">
        ...
        <div class="main">
          ...
          <p class="tig" v-if="item.PromotionalLabels"><span class="blue" v-if="item.PromotionalLabels.length>0">{{item.PromotionalLabels[0]}}</span><span class="green" v-if="item.PromotionalLabels.length>=2">{{item.PromotionalLabels[1]}}</span><span class="orange" v-if="item.PromotionalLabels.length>=3">{{item.PromotionalLabels[2]}}</span></p>
        </div>
        <div class="icon"></div>
    </li>
  </ul>
    <ul v-if="index==0&&houselist.length>0">
    <li v-for="(item,index) in houselist" @click="detail(2,item.ID,index)" :class="{out:item.SaleState=='已售'||item.SaleState=='停售'}">
        <div class="img"><img :src="item.Img"></div>
        <div class="main">
          <p class="title">{{item.Name}}</p>
          <p class="add">地址：{{item.HousesAddress}}</p>
          <p class="tig" v-if="item.Tag"><span class="blue" v-if="item.Tag.length>0">{{item.Tag[0]}}</span><span class="green" v-if="item.Tag.length>=2">{{item.Tag[1]}}</span><span class="orange" v-if="item.Tag.length>=3">{{item.Tag[2]}}</span></p>
          <p class="money"><span class="red">{{item.Price}}</span> {{item.BuildingSpace}}</p>
        </div>
        <div class="icon"></div>
    </li>
  </ul>
    <p style="text-align:center; height:1.5rem; line-height:1.25rem;" v-if="showmoer">{{loaddata}}</p>
...
export default {
  ...
  methods:{
    ...
    detail(type,id,index){
      if(type==1){
        if(this.houselist[index].HouseSaleStatus=='已售'||this.houselist[index].HouseSaleStatus=='停售'){
          return
        }
        this.$router.push("/oldhousedetail?id="+id)
      }else{
        if(this.houselist[index].SaleState=='已售'||this.houselist[index].SaleState=='停售'){
          return
        }
    ...
    gethouselist(){
     ...
     if(this.index==0){
          api=this.$api.GetHouseGetStoreups;
      }
      if(this.index==1){
          api=this.$api.GetHouseStoreups;
      }

      this.$http(api({"Page":this.page,"PageSize":5}))
      .then((data) => {
  ...
  created :function(){
    let _this=this
    if(this.$store.state.isapp=='ios'){
      this.styletop = "top:1.15rem"
    }
    if(this.$store.state.isapp=='android'){
      this.styletop = "top:1.2rem"
    }
    this.gethouselist(this.index);
...
.nav{display:flex}
.nav .box{position:relative;height:rem(45);line-height:rem(45);border-bottom:1px solid #d8d8d8;background:#fff;}
.nav a{height:rem(45);line-height:rem(45);text-align:center; font-size:0.45rem;display:block; color:#888;}
.nav a.active{color:#de3031;border-bottom:1px solid #de3031;}
...
.hothouse_box{background:#fff;padding:rem(15);box-sizing:border-box;position: absolute;z-index: 1;top: 2.25rem;bottom: 0;overflow: hidden;width: 100%;touch-action: none;}
...
.hothouse_box li.out{position:relative;}
.hothouse_box li.out .icon{display:block;width:rem(80);height:rem(80); position:absolute;top:rem(30);right:rem(10);@include bg("/static/img/house/out.png");}
.hothouse_box li.out .img{width:rem(120);height:rem(100); float:left;}
.hothouse_box li.out .img img{border-radius:rem(5);width:100%;height:100%;}
.hothouse_box li.out .main{overflow:hidden; width:auto; padding-left:rem(15)}
.hothouse_box li.out .main p.title{font-size:0.4rem;color:#dcdcdc}
.hothouse_box li.out .main p.add{color:#dcdcdc; font-size:0.3rem;margin-top:rem(5);overflow:hidden;font-size:0.35rem;}
.hothouse_box li.out .main p.add span{float:right;color:#dcdcdc;width:auto;}
.hothouse_box li.out .main p.tig{overflow:hidden;margin-top:rem(5);}
.hothouse_box li.out .main p.tig span{float:left;margin-right:rem(5); width:auto; padding:rem(2) rem(5);border-radius:rem(5);}
.hothouse_box li.out .main p.tig span.blue{background:#dcdcdc; color:#fff;}
.hothouse_box li.out .main p.tig span.green{background:#dcdcdc; color:#fff;}
.hothouse_box li.out .main p.tig span.orange{background:#dcdcdc; color:#fff;}
.hothouse_box li.out .main p.money{margin-top::rem(5);color:#dcdcdc;}
.hothouse_box li.out .main p.money span{font-size:0.4rem;color:#dcdcdc;}
